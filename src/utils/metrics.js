...
export const getDateWindow = (days) => {
  const end = new Date()
...
export const getPreviousWindow = (days) => {
...
export const formatVariance = (value) => `${value >= 0 ? '+' : ''}${Number(value).toFixed(1)}%`
...
export const getCostTone = (value) => {
...
export function filterShipments(shipments, filters) {
  ...
  return shipments.filter((shipment) => {
    const shipDate = new Date(shipment.date_created)

    if (shipDate < start || shipDate > end) {
      return false
    }

    if (filters.region && filters.region !== 'all' && shipment.region !== filters.region) {
      return false
    }

    if (filters.carrier && filters.carrier !== 'all' && shipment.carrier_id !== filters.carrier) {
      return false
    }
...
export function getTrendDelta(current, previous) {
  if (!previous || previous === 0) return 0
  return Number((((current - previous) / previous) * 100).toFixed(1))
}

export function getSummaryMetrics(shipments, exceptions, filters = {}) {
  ...
  const delivered = activeShipments.filter((shipment) => shipment.status === 'Delivered')
  const onTimeCount = delivered.filter((shipment) => {
  ...
  const previousShipments = shipments.filter((shipment) => {
    const shipDate = new Date(shipment.date_created)
    return shipDate >= previousWindow.start && shipDate <= previousWindow.end
  })
...
export function getCarrierScorecard(shipments, exceptions, filters = {}) {
...
export function getExceptionTrendData(shipments, exceptions, filters, view = 'type', metric = 'count') {
  const relevantShipments = filterShipments(shipments, filters)
  const days = Number(filters.dateRange || 30)
  const points = Array.from({ length: days >= 90 ? 12 : 7 }, (_, index) => {
    const label = days >= 90 ? `W${index + 1}` : `D${index + 1}`
    return { label, late: 0, docs: 0, invoice: 0 }
  })
  ...
  if (view === 'carrier') {
    return carriers.map((carrier) => {
      ...
      const carrierExceptions = exceptionBuckets.filter((exception) => exception.carrier_id === carrier.id)
      carrierExceptions.forEach((exception) => {
...
export function getRegionalSummary(shipments, exceptions, filters = {}) {
  const relevantShipments = filterShipments(shipments, filters)
  const regionNodes = ['Northeast', 'Southeast', 'Midwest', 'West', 'Southwest'].map((region) => {
  ...
  return regionNodes.sort((a, b) => a.onTimeRate - b.onTimeRate)
}

export function getOpenExceptions(shipments, exceptions, filters = {}) {
  const relevantShipments = filterShipments(shipments, filters)
  return exceptions
...
export const regionOrder = ['Northeast', 'Southeast', 'Midwest', 'West', 'Southwest']
